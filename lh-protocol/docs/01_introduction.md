...
- The expected daily fee rate (used to size `y·E[F]`) is derived from **Birdeye pool volume + TVL** via `orca-volume-adapter.ts`, composed with an on-chain concentration factor `c(width)` read from `whirlpool.liquidity`. *(Scope note, 2026-07-28: this describes the PoC prototype. The shipping platform measures fee yield directly from its own 15-minute `feeGrowthGlobal` snapshots and, per position, from `feeGrowthInside` — Birdeye survives only as a labelled fallback; the quote path currently sizes `y·E[F]` from a conservative governance constant.)*
