 ...
 *   C1. `computeQuote().fairValueUsdc == computeQuadratureFV_E6(...)` exactly.
...
import { clPositionValue } from "../../src/pricing-engine/position-value";
import { calibrateSeverityForPool } from "../../src/risk-analyser/regime";
import { DEFAULT_MARKUP_FLOOR, PPM } from "../../src/types";
import type {
  PoolState,
  RegimeSnapshot,
  TemplateConfig,
...
describe("Formula-bound premium with real Gauss-Hermite FV", () => {
  ...
  describe("Model-consistency contract (off-chain quote ≡ GH quadrature)", () => {
    ...
    for (const t of tenors) {
      it(`C1. computeQuote.fairValueUsdc === computeQuadratureFV_E6 (at ${t.label})`, () => {
        const tpl: TemplateConfig = { ...TEMPLATE, tenorSeconds: t.seconds };
        const ghDirect = computeQuadratureFV_E6(
  ...
  it("real Gauss-Hermite FV is at least an order of magnitude larger than the heuristic", () => {
    const ghFV = computeQuadratureFV_E6(
      Math.floor(S0 * 1_000_000),
      SIGMA_PPM,
      NORMALISED_L,
      Math.floor(P_L * 1_000_000),
...
function buildSweepRows(
  ...
  return tenors.map((t) => {
    ...
    const gh = computeQuadratureFV_E6(
      Math.floor(S0 * 1_000_000),
      SIGMA_PPM,
      NORMALISED_L,
      Math.floor(P_L * 1_000_000),
