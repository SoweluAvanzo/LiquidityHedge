...
/**
 * Convert an on-chain u128 quantity (e.g. Whirlpool liquidity) to a JS
 * number for use in the float-domain value functions.
 *
 * This is the ONLY sanctioned bigint→number cast in @lh/core (SR-1).
 * Above Number.MAX_SAFE_INTEGER (2^53−1) the cast silently loses
 * precision, so we throw instead: callers dealing with larger values
 * must normalize (e.g. scale by token decimals) before valuation.
 */
export function u128ToNumber(value: bigint): number {
  if (value < 0n) {
    throw new Error(`u128ToNumber: negative value ${value}`);
  }
  if (value > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error(
      `u128ToNumber: ${value} exceeds 2^53-1; normalize before casting to avoid silent precision loss`,
    );
  }
  return Number(value);
}
