 ...
 *
 * v2 changes:
 * - initPool accepts premiumMode and twoPartAlpha in config
 * - PoolState carries premiumMode and twoPartAlpha fields
 * - depositUsdc / withdrawUsdc logic is unchanged from v1
 */
 ...
 * @param config  Full pool initialization config (includes premiumMode, twoPartAlpha)
 */
export function initPool(
  ...
  const pool: PoolState = {
    ...
    twoPartAlpha: config.twoPartAlpha ?? DEFAULT_ALPHA,
 ...
 *
 * Premiums flowing into reserves cause share value to rise over time,
 * so later depositors get fewer shares per USDC — this is the RT income
 * mechanism.
 *
